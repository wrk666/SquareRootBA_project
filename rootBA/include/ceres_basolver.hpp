...
class ceresBAProblem{
    ...
    ceresBAProblem(std::vector<Frame>& cameras, std::vector<Eigen::Matrix<Scalar, 3, 1>>& points):problem()
    {
        ...
        //在堆上开辟内存存放数据，析构时手动删除
        lm_pose_ = new double*[lm_num_];//指针数组
        ...
        for (int i = 0; i < frame_num_; i++)
        {
            ...
            //ceres显式添加parameterBlock，实际上landmark pos也参与优化
            problem.AddParameterBlock(c_rotation_[i], 4, local_parameterization);//用ceres的四元数去冗余
