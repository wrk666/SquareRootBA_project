...
namespace rootBA {
    ...
    private:
        ...
        // LM 算法调整阻尼因子的相关参数()包含strategy1和3
        Scalar Lup;
        Scalar Ldown;
        Scalar v;
        Scalar rho;
        ...
        int invalidCount = 0;
        // 记录最小误差
        std::pair<int, Scalar> minCost;//iter轮数，min_cost
...
namespace rootBA {
    ...
    template<typename Scalar> ProblemVO<Scalar>::ProblemVO() {
        this->Reset();//清空所有保存的信息
    }
    template<typename Scalar> ProblemVO<Scalar>::~ProblemVO() {}//为啥不release相关内存？哦，因为是shared_ptr，所以不引用会自动被delete
    ...
    bool ProblemVO<Scalar>::AddExPose(const ExPosePtr &exPose) {
        if (exPose == nullptr) {
            ...
            Vector3<Scalar> t_bc;
            t_bc.setZero();
            ExPosePtr newExPose(new VertexExPose<Scalar>(q_bc, t_bc));//new分配在堆上，且exPose是智能指针，不会被销毁
            this->exPose = newExPose;//exPose是智能指针，指的内存不会被销毁（只能手动delete），但是newExPose这个局部指针变量会被析构
        } else {
            this->exPose = exPose;
        }
        return true;
...
namespace rootBA {
    ...
    bool ProblemVO<Scalar>::Solve(size_t maxIteration) {
        ...
        this->PrecomputeResidual();//tbb并行计算
        ...
        for (size_t iter = 0; iter < maxIteration; ++iter) {
            ...
            if (this->linearizedCost - this->sumCost < this->linearizedCost * this->threshold_costRate &&
                ...
                printf("this->linearizedCost - this->sumCost: %f, this->threshold_costRate: %f, this->linearizedCost * this->threshold_costRate: %f, "
                       "this->linearizedCost: %f, this->sumCost: %f\n", this->linearizedCost - this->sumCost, this->threshold_costRate, this->linearizedCost * this->threshold_costRate,
                       this->linearizedCost, this->sumCost);
                break;
            }
            if (this->normDeltaX < this->threshold_normDelta) {
                std::cout << "<End> norm delta X ( " << this->normDeltaX <<" ) up to threshold( " << this->threshold_normDelta <<" ), finished." << std::endl;
                break;
            }
            // 更新最小误差，并判断是否终止迭代
            ...
            if (iter - this->minCost.first > this->threshold_minCostHold) {  //距离最小误差大于一轮就结束？
                std::cout << "<End> min cost holding times up to threshold, finished." << std::endl;
                printf("iter: %d, min cost iter: %d, this->threshold_minCostHold: %d\n", iter, this->minCost.first, this->threshold_minCostHold);
                break;
            }
    ...
    // 在并行计算中，"规约"（Reduction）指的是将多个值合并成一个值的过程。典型的规约操作包括对一组数值进行求和、求平均值、找到最大或最小值等。
    /* 遍历每一个 landmark block，基于当前相机 pose 和特征点的 position 来预计算误差 r */
    /* 同时，计算 vo problem 的整体误差 */
    ...
    bool ProblemVO<Scalar>::PrecomputeResidual(void) {
        this->sumCost = tbb::parallel_reduce(
                tbb::blocked_range<size_t>(0, landmarkBlocks.size()),// 参数1：迭代范围(所有landmarkBlocks)
                Scalar(0), // 参数2：初始值
                // 参数3：lambda 函数，定义局部操作
                [&] (tbb::blocked_range<size_t> range, Scalar localSum) {
                        for (size_t i = range.begin(); i < range.end(); ++i) {//一个landmarkBlock中的所有camera观测
        ...
        this->rowsof_matrix_J += tbb::parallel_reduce(tbb::blocked_range<size_t>(0, landmarkBlocks.size()), Scalar(0),
            [&] (tbb::blocked_range<size_t> range, Scalar temp) {
                for (size_t i = range.begin(); i < range.end(); ++i) {
                    // 清空 storage 矩阵
                    this->rowIndices[i] = this->landmarkBlocks[i]->ResizeStorage();//local的landmarkBlocks行数，不包含landmark damp
...
//        size_t i = 0;
//        size_t idx = 0;
//        std::cout << "\nstorage<2, 6> Jex:\n" << this->landmarkBlocks[i]->storage.template block<2, 6>(idx * 2, 0)
//                  << "\nstorage<2, 3> Jl:\n" << this->landmarkBlocks[i]->storage.template block<2, 3>(idx * 2, this->landmarkBlocks[i]->storage.cols() - 4)
//                  << "\nstorage<2, 6> Jp:\n" << this->landmarkBlocks[i]->storage.template block<2, 6>(idx * 2, 6 + idx * 6)
//                  << std::endl;

        //rowIndices是所有landmark blocks的local行数，将local变为global行数，第一个起始为0
    ...
    bool ProblemVO<Scalar>::InitializeLM(void) {
        this->lambda = 5e1;//1e-6
        this->linearizedCost = this->sumCost;
        this->v = 2.0;//strategy 3 的参数
        this->invalidCount = 0;
        return true;
    }
    ...
    bool ProblemVO<Scalar>::SolveIncrementalFunction(void) {
        ...
        this->delta_X.head(this->matrix_H.cols()) = this->SolveLinearFunction(this->matrix_H, this->vector_b);//求出Delta_xp
                ...
                for (size_t i = range.begin(); i < range.end(); ++i) {
                    this->delta_X.segment(this->matrix_H.rows() + i * 3, 3) =
                        this->landmarkBlocks[i]->ComputeDeltaXl(
                            this->delta_X.head(this->matrix_H.cols()));//求出Delta_xl
                }
            }
        );
