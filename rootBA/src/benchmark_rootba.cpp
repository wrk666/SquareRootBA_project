...
void GetSimDataInWordFrame(std::vector<Frame>& cameraPoses, std::vector<rootBA::Vector3<Scalar>>& points, int featureNums,int poseNums)
{
    ...
    for (int n = 0; n < poseNums; ++n) {
        ...
        // radius * cos(theta) 表示圆上一点的 x 坐标，而减去 radius 的目的是将圆心移到 (radius, 0, 0) 处，以便整个分布是围绕 (radius, 0, 0) 进行的
        // radius * sin(theta) 计算 y 分量，其中 radius * sin(theta) 表示圆上一点的 y 坐标。
        // 1 * sin(2 * theta) 计算 z 分量。在这里，通过使用 sin(2 * theta)，作者可能希望在 z 轴上引入一些随时间变化的振荡。
        rootBA::Vector3<Scalar> t = rootBA::Vector3<Scalar>(radius * cos(theta) - radius, radius * sin(theta), 1 * sin(2 * theta));
        cameraPoses.push_back(Frame(R, t));//Twc,同时也计算了Tcw
    }
...
void AddNoiseinCamera(std::vector<Frame> &cameras)
{
    ...
    for (size_t i = 0; i < cameras.size(); ++i) {
        ...
        cameras[i].setNoise(noise_t, noise_q_wc);//qwc,twc，qcw,tcw添加噪声
    }
}
...
int main() {
    ...
    ceresBAProblem ceres_problem(cameras,noisy_points);//ceres solver作为对比，ceres这里只使用double
    ceres_problem.set_options();
    rootBA::TicToc timer;
    ceres_problem.solve();
    auto ceres_time = timer.toc();
    std::cout << "<ceres> cost time ms" << ceres_time << std::endl;
    ...
    std::shared_ptr<rootBA::VertexExPose<Scalar>> exPose(new rootBA::VertexExPose<Scalar>(q_bc, t_bc));//外参
    problem.AddExPose(exPose);//以智能指针指向pose的内存，ProblemVO对象析构后该片pose的内存会被自动释放
    exPose->SetFixed();
...
//    std::default_random_engine generator;
//    std::normal_distribution<double> camera_rotation_noise(0., 0.1);//这个噪声好像没有用到
//    std::normal_distribution<double> camera_position_noise(0., 0.2);
    ...
    problem.SetThreshold(1e-6, 20, 1e-6, 1); // 设置终止迭代判断阈值
    ...
    exPose->SetFixed(true); //设为外参设为Identity，不进行优化
    problem.SetDampPolicy(rootBA::ProblemVO<Scalar>::DampPolicy::Auto);//policy3
...
std::cout << "\nStep 8: Compute average residual." << std::endl;
    ...
    std::cout << "  Average camera atitude residual at initialization is " << residual_init / Scalar(cameras.size()) << " rad, " <<
            residual_init/ Scalar(cameras.size()) * 57.29578049 << " deg\n" << std::endl;
